...
SAPO.namespace('Dom');SAPO.Dom.Css={addRemoveClassName:function(elm,className,addRemState){if(addRemState){return SAPO.Dom.Css.addClassName(elm,className);}
...
return elm;}
return null;},create:function(tag,properties){var el=document.createElement(tag);SAPO.extendObj(el,properties);return el;},scrollTo:function(elm){elm=this.get(elm);if(elm){if(elm.scrollIntoView){return elm.scrollIntoView();}
...
this._win=win;this._doc=win.document;this._root=this._doc.documentElement;this._done=false;this._top=true;this._handlers={checkState:this._checkState.bindObjEvent(this),poll:this._poll.bindObj(this)};var ael=this._doc.addEventListener;this._add=ael?'addEventListener':'attachEvent';this._rem=this._doc.addEventListener?'removeEventListener':'detachEvent';this._pre=this._doc.addEventListener?'':'on';var csf=this._handlers.checkState;this._cbQueue.push(fn);if(this._doc.readyState==='complete'){fn.call(this._win,'lazy');}
else if(this._doc.addEventListener){this._doc[this._add]('DOMContentLoaded',csf,false);this._win[this._add]('load',csf,false);}
else{this._doc[this._add]('onreadystatechange',csf);this._win[this._add]('onload',csf);if(this._root.doScroll&&this._root.doScroll){try{this._top=!this._win.frameElement;}catch(e){}
...
(function(undefined){'use strict';SAPO.namespace('Ink');var Aux=SAPO.Ink.Aux,Css=SAPO.Dom.Css,Element=SAPO.Dom.Element,Event=SAPO.Dom.Event;var Modal=function(selector,options){this._options=SAPO.extendObj({width:600,height:400,markup:undefined,onShow:undefined,onDismiss:undefined,closeOnClick:false,skipDismiss:false,resize:false,disableScroll:false},options||{});this._handlers={click:this._onClick.bindObjEvent(this),keyDown:this._onKeyDown.bindObjEvent(this)};if(this._options.markup===undefined){this._referenceElement=Aux.elOrSelector(selector,'1st argument');}
this._wasDismissed=false;this._init();};Modal.prototype={_init:function(){var contentMarkup=this._options.markup!==undefined?this._options.markup:this._referenceElement.innerHTML;var s;var div2El=document.createElement('div');div2El.className='ink-modal ink-space';s=div2El.style;s.position='absolute';var w=this._options.width;var h=this._options.height;w=this._options.width;s.width=w+'px';h=this._options.height;s.height=h+'px';s.marginLeft=['-',~~(w/2),'px'].join('');s.marginTop=['-',~~(h/2),'px'].join('');s.left='50%';s.top='50%';div2El.innerHTML='';var divEl=document.createElement('div');divEl.className='ink-shade';s=divEl.style;s.position='fixed';s.left=0;s.right=0;s.top=0;s.bottom=0;var divContentContainer=document.createElement('div');divContentContainer.className='ink-modal-content';divContentContainer.innerHTML=[(this._options.skipClose?'':'<a href="#" class="ink-close">×</a>'),contentMarkup].join('');divEl.appendChild(div2El);document.body.appendChild(divEl);div2El.appendChild(divContentContainer);this._contentElement=div2El;this._contentContainer=divContentContainer;this._shadeElement=divEl;if(this._options.onShow){this._options.onShow(this);}
if(this._options.resize){this._resizeContainer();}
if(this._options.disableScroll){this._disableScroll();}
Event.observe(this._shadeElement,'click',this._handlers.click);Event.observe(document,'keydown',this._handlers.keyDown);Aux.registerInstance(this,this._shadeElement,'modal');},_onClick:function(ev){var tgtEl=Event.element(ev);if(Css.hasClassName(tgtEl,'ink-close')||(this._options.closeOnClick&&!Element.descendantOf(this._shadeElement,tgtEl))){Event.stop(ev);this.dismiss();}},_onKeyDown:function(ev){if(ev.keyCode!==27||this._wasDismissed){return;}
this.dismiss();},_resizeContainer:function()
{var contentHeight=SAPO.Dom.Element.elementHeight(this._contentContainer);this._contentElement.style.overflow=this._contentElement.style.overflowX=this._contentElement.style.overflowY='visible';if(contentHeight>SAPO.Dom.Element.elementHeight(this._contentElement)){this._contentElement.style.height=contentHeight+'px';var marginTop=~~(contentHeight/2);this._contentElement.style.marginTop=['-',marginTop,'px'].join('');var contentPos=SAPO.Dom.Element.offset2(this._contentElement);if(contentPos[1]<0){marginTop-=(Math.abs(contentPos[1])+20);contentHeight-=(Math.abs(contentPos[1])*2+30);this._contentElement.style.marginTop=['-',marginTop,'px'].join('');this._contentElement.style.height=contentHeight+'px';this._contentElement.style.overflow='auto';this._contentElement.style.overflowX='hidden';if(!this._options.skipClose){var aClose=SAPO.Dom.Selector.select('.ink-close',this._contentElement);aClose[0].style.top='5px';aClose[0].style.right='5px';}}}else{this._contentElement.style.height=this._options.height+'px';var marginTop=~~(parseInt(this._options.height)/2);console.log(parseInt(this._options.height));this._contentElement.style.marginTop=['-',marginTop,'px'].join('');var contentPos=SAPO.Dom.Element.offset2(this._contentElement);if(contentPos[1]<0){marginTop-=(Math.abs(contentPos[1])+20);contentHeight-=(Math.abs(contentPos[1])*2+30);this._contentElement.style.marginTop=['-',marginTop,'px'].join('');this._contentElement.style.height=this._options.height;if(!this._options.skipClose){var aClose=SAPO.Dom.Selector.select('.ink-close',this._contentElement);}}}},_disableScroll:function()
{this._oldScrollPos=SAPO.Dom.Element.scroll();this._onScrollBinded=function(event){window.scrollTo(this._oldScrollPos[0],this._oldScrollPos[1]);}.bindObjEvent(this);SAPO.Dom.Event.observe(window,'scroll',this._onScrollBinded);},dismiss:function(){if(this._options.onDismiss){this._options.onDismiss(this);}
if(this._options.disableScroll){SAPO.Dom.Event.stopObserving(window,'scroll',this._onScrollBinded);}
var el=this._shadeElement;this._wasDismissed=true;el.parentNode.removeChild(el);Aux.unregisterInstance(this._instanceId);},getContentElement:function(){return this._contentContainer;},setContentMarkup:function(contentMarkup){this._contentElement.innerHTML='';this._contentElement.appendChild(this._contentContainer);this._contentContainer.innerHTML=[this._options.skipClose?'':'<a href="#" class="ink-close">×</a>',contentMarkup].join('');this._resizeContainer();}};Modal.destroy=Modal.dismiss;SAPO.Ink.Modal=Modal;})();
